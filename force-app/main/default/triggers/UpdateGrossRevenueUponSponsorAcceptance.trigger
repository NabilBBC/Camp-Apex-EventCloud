trigger UpdateGrossRevenueUponSponsorAcceptance on CAMPX__Sponsor__c(
  after insert,
  after update
) {
    ...
    for (CAMPX__Sponsor__c sponsor : Trigger.new) {
      if (
        ...
        AggregateResult result = [
          SELECT SUM(CAMPX__ContributionAmount__c) total
        ...
        eventsToUpdate = [
          SELECT CAMPX__GrossRevenue__c, name
          FROM CAMPX__Event__c
          WHERE id = :sponsor.CAMPX__Event__c
        ];

        if (!eventsToUpdate.isEmpty()) {
             eventsToUpdate[0].CAMPX__GrossRevenue__c = totalAmountOfContributions;
        }
      }
      if (!eventsToUpdate.isEmpty()) {
    ...
    if (Trigger.isUpdate) {
      for (CAMPX__Sponsor__c oldSponsor : Trigger.old) {
        ...
        if (
          ...
          (Trigger.new.get(0).CAMPX__Status__c == 'Pending' ||
            Trigger.new.get(0).CAMPX__Status__c == 'Rejected') ||
            Trigger.new[0].CAMPX__Event__c != oldSponsor.CAMPX__Event__c)
         {
            eventsToUpdate = [
